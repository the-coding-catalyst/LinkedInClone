...
const createABlog = async(req, res, next) => {
    ...
    let blogUser 
    try{
        blogUser = await User.findById(user)
    }catch(err){
        return console.log(err)
    }
    ...
    let newBlog
    try{
       newBlog = await new Blog({title, description, user})
    }catch(err){
        return console.log(err)
    }
...
const getBlogByUser = async (req, res, next) => {
    let blog
    try{
        blog = await User.findById(req.params.id).populate("blogs")
    }catch(error){
        console.log(error)
...
module.exports = {getAllBlogs, createABlog, deleteABlog, updateABlog, getBlogById, getBlogByUser}